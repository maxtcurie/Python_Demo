...
def spectral_density(function,time,percent=0.5,window_for_FFT='boxcar',plot=False):
	time=np.array(time)
	dt=time[1:]-time[:-1]
	dt_min=np.min(dt)
	...
	f, Pxx_den = signal.welch(uni_function, fs, nperseg=int(percent*len(uni_function)), window=window_for_FFT) #, scaling='spectrum')
...
f, Pxx_den=spectral_density(function,time,percent=0.5,window_for_FFT=window_input,plot=True)
